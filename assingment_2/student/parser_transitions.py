...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        # У нас есть:
        # - sentence: список слов предложения
        # Нужно инициализировать:
        # - self.stack: текущий стек
        # - self.buffer: текущий буфер
        # - self.dependencies: список зависимостей

            # Стек начинается с ROOT
        self.stack = ["ROOT"]

            # Буфер начинается со всех слов предложения
        self.buffer = sentence[:]  # делаем копию!
    ...
    def parse_step(self, transition):
        ...
        ### YOUR CODE HERE (~7-12 Lines)
        if transition == "S":
            # SHIFT: берем слово из буфера в стек
            if self.buffer:
                word = self.buffer.pop(0)
                self.stack.append(word)

        elif transition == "LA":
        # LEFT-ARC: stack[-1] → stack[-2]
            if len(self.stack) >= 2:
                head = self.stack[-1]      # ПОСЛЕДНИЙ элемент - голова
                dependent = self.stack[-2]  # ВТОРОЙ с конца - зависимое
                self.dependencies.append((head, dependent))
                self.stack.pop(-2)          # удаляем зависимое (второй с конца)

        elif transition == "RA":
            # RIGHT-ARC: stack[-2] → stack[-1]
            if len(self.stack) >= 2:
                head = self.stack[-2]      # ВТОРОЙ с конца - голова
                dependent = self.stack[-1]  # ПОСЛЕДНИЙ - зависимое
                self.dependencies.append((head, dependent))
                self.stack.pop(-1)          # удаляем зависимое (последний)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Создаем начальные partial parses
    partial_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = partial_parses[:]  # поверхностная копия

    while unfinished_parses:
        # Берем min(batch_size, len(unfinished_parses)) парсеров
        batch = unfinished_parses[:batch_size]

        # Получаем предсказания от модели
        transitions = model.predict(batch)

        # Применяем переходы к каждому парсеру в батче
        for i, parse in enumerate(batch):
            parse.parse_step(transitions[i])

        # Удаляем завершенные парсеры (только ROOT в стеке и пустой буфер)
        unfinished_parses = [parse for parse in unfinished_parses
                             if not (len(parse.buffer) == 0 and len(parse.stack) == 1)]

    # Собираем результаты из исходного списка partial_parses
